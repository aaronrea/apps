 ...
 *   2. Icons: cache-first. They never change.
 *   3. Audio streams: network-only, never cached, never even looked up in the
 ...
 * refresh: the page sends {type:'check-update'} on launch and whenever it
 * comes back to the foreground. The worker re-fetches every shell file —
 * unchanged ones answer 304 and cost nothing — and if any of them differs
 * from the cached copy it stores the new one and posts {type:'update-ready'}
 * back, which makes the page reload itself onto the new code, once nothing is
 * playing. See VERSION_KEY below for why that reload lands where it should.
 *
